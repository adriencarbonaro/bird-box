...
static void supervisor_task(void *arg)
{
    ...
    while (1)
    {
        ...
        uint32_t bits = xEventGroupWaitBits(stop_event_group,
                                            BIT(STOP_REASON_STREAM_OPEN_FAILED) |
                                            BIT(STOP_REASON_AUDIO_ENDED),
                                            pdTRUE,
                                            pdFALSE,
                                            pdMS_TO_TICKS(10));
        if (bits & (BIT(STOP_REASON_STREAM_OPEN_FAILED) | BIT(STOP_REASON_AUDIO_ENDED)))
        {
            ESP_LOGI(TAG, "Stop indication (bits=0x%08x)", bits);
            stop();
        }
    }
}
...
void supervisor_init(void)
{
    ...
    init_pipeline();
    stream_init(stop_event_group);
    audio_init(stop_event_group);
    mp3_decode_init();
